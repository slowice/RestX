...
function makeApi(consent: boolean): RestXApi {
  return {
    inspector: {
      chooseDirectory: vi.fn(), scanDirectory: vi.fn(), readConfig: vi.fn(), readJsonlPage: vi.fn(), readJsonlEntry: vi.fn(), revealInFolder: vi.fn()
    },
    app: {
    ...
    ai: {
      ...
      getProviderSettings: vi.fn(async (): Promise<AiProviderPublicSettings> => ({ provider: 'openai-compatible', baseUrl: 'https://example.com/v1', model: 'demo', apiKeyConfigured: true })),
      updateProviderSettings: vi.fn(), analyzeConfig: vi.fn(),
