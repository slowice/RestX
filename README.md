...
RestX 会自动创建 `~/.RestX/log`，并按本地日期写入 `ai-calls-YYYY-MM-DD.jsonl`。日志时间包含本地时区偏移（例如中国标准时间为 `+08:00`）。日志包含：
...
预置定义位于 `src/main/presets/ai-tools/`：
...
用户预置位于 `~/.RestX/presets/`，支持 `.json`、`.yaml` 和 `.yml`。智能导入会生成 JSON；手工编辑的 YAML 使用同一契约。单个损坏预置会在管理页面显示错误，不会阻断其他预置。

通用框架位于 `src/main/services/ai-tool-discovery.ts`，不包含具体工具 id 判断。新预置无需修改扫描器、IPC 或页面组件。
...
第一阶段基础边界见 [docs/phase-1-foundation.md](docs/phase-1-foundation.md)，本次配置解析变更见 [OpenSpec design](openspec/changes/add-config-view-ai-analysis-cache/design.md)。
